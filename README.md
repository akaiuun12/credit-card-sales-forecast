...
- **결과**: Recency Weighting(2020~2023 연도별 가중치 0.1→0.4) 적용 후 최종 테스트(2024년 1월)에서 **LightGBM**이 MSE 17,749, 오차율 -0.92%로 가장 우수한 성능을 보였습니다. CV와 Test 모두 LightGBM이 가장 낮은 오차를 기록했습니다.
...
Recency Weighting 적용 후 최종 테스트(2024년 1월)에서 **LightGBM**이 MSE 17,749, 오차율 -0.92%로 가장 낮은 오차를 기록해 최종 모델로 선정되었습니다. CV와 Test 모두에서 LightGBM이 가장 우수한 성능을 보였습니다.

| 모델               |           CV MSE |          CV MAE |         Test MSE |        Test MAE |      예측 오차율 |
| ------------------ | ---------------: | --------------: | ---------------: | --------------: | ---------------: |
| **LightGBM** | **12,972** | **72.99** | **17,749** |           89.99 | **-0.92%** |
| XGBoost            |           13,294 |           74.79 |           19,514 |           88.57 |           -6.36% |
| RandomForest       |           14,841 |           79.69 |           21,379 | **82.26** |           -2.01% |

실제 매출액 27,100억 원 대비 249억 원(0.92%) 과소 추정으로, 월간 목표 관리에 활용 가능한 수준을 달성했습니다.
